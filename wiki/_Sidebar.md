# SigProfilerMatrixGenerator

## Getting Started
- [Home](Home)
- [Quick Start Example](Quick-Start-Example)

## Installation
- [Installation - Python](Installation-Python)
- [Installation - R](Installation-R)
- [Currently Supported Genomes](Currently-Supported-Genomes)

## Using the Tool
- [SBS, ID, DBS Input](Using-the-Tool-SBS-ID-DBS-Input)
- [CNV Input](Using-the-Tool-CNV-Input)
- [SV Input](Using-the-Tool-SV-Input)
- [Output Overview](Using-the-Tool-Output)

## Output Details
- [Output - SBS](Output-SBS)
- [Output - DBS](Output-DBS)
- [Output - ID](Output-ID)
- [Output - TSB](Output-TSB)
- [Output - vcf_files](Output-vcf_files)
- [Output - Plots](Output-Plots)

## Resources
- [GitHub Repository](https://github.com/SigProfilerSuite/SigProfilerMatrixGenerator)
- [SigProfilerPlotting](https://github.com/AlexandrovLab/SigProfilerPlotting)
- [Contact](mailto:contact@sigprofilersuite.org)
