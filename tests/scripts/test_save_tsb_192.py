...
def test_save_tsb_handles_nested_opposite_strand_transcripts(tmp_path):
    ...
    save_tsb_192.save_tsb(f"{chromosome_dir}/", f"{transcript_dir}/", f"{output_dir}/")
...
def test_save_tsb_preserves_inclusive_one_base_transcript(tmp_path):
    ...
    (transcript_dir / "I_transcripts.txt").write_text("gene1\ttx1\tI\t-1\t3\t3\n")
...
def test_save_tsb_does_not_replace_existing_output_after_failure(tmp_path):
    ...
    (transcript_dir / "I_transcripts.txt").write_text("gene1\ttx1\tI\t1\t2\t20\n")
...
def test_save_tsb_matches_per_base_oracle_on_randomized_intervals(tmp_path):
    rng = random.Random(251)
    chromosome_dir, transcript_dir, output_dir = make_reference(tmp_path)
    for _ in range(100):
        length = rng.randint(1, 80)
        sequence = "".join(rng.choices("ACGTN", k=length))
        intervals = []
        for _ in range(rng.randint(0, 30)):
            start = rng.randint(1, length)
            intervals.append(
                (start, rng.randint(start, length), rng.choice(["1", "-1"]))
            )
        (chromosome_dir / "I.txt").write_text(sequence)
        (transcript_dir / "combined.txt").write_text(
            "".join(
                f"g{i}\tt{i}\tI\t{strand}\t{start}\t{end}\n"
                for i, (start, end, strand) in enumerate(intervals)
            )
        )
        save_tsb_192.save_tsb(chromosome_dir, transcript_dir, output_dir)
        observed = (output_dir / "I.txt").read_bytes()
        assert decode_bases(observed) == sequence
        expected = []
        # Independent implementation: check interval membership at each base.
        ...
        assert decode_states(observed) == "".join(expected)
